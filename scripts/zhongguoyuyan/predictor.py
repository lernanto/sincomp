...
class Predictor(tf.train.Checkpoint):
    ...
    def __init__(
        ...
        super().__init__(step=tf.Variable(0, dtype=tf.int64))
    ...
    def train(
        ...
        while self.step < epochs:
            ...
            with writer.as_default():
                tf.summary.scalar(train_loss.name, train_loss.result(), step=self.step)
                tf.summary.scalar(train_acc.name, train_acc.result(), step=self.step)
                tf.summary.scalar(eval_loss.name, eval_loss.result(), step=self.step)
                tf.summary.scalar(eval_acc.name, eval_acc.result(), step=self.step)
...
if __name__ == '__main__':
    ...
    char = pandas.read_csv(os.path.join(prefix, 'words.csv'), index_col=0).sample(200)
