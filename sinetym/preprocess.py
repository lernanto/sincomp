...
_CHAR_MAP = {
    ...
    0x00a4: 0x0235, # CURRENCY SIGN -> LATIN SMALL LETTER N WITH CURL
    ...
    0xf20d: 0x0264, # -> LATTIN SMALL LETTER RAMS HORN
...
def clean_tone(raw: pandas.Series) -> pandas.Series:
    ...
    return raw.str.replace(f'[^{"".join(_TONES)}]', '', regex=True)
...
def normalize_initial(origin: pandas.Series) -> pandas.Series:
    ...
    return origin.str.replace('\u02a3', 'dz', regex=False) \
